...
const main = async () => {
  ...
  helia.libp2p.handle('/hashchan/janny/1.0.0', async (stream, connection) => {
    try {
      ...
      console.log('[janny] received from', connection.remotePeer.toString(), json)
      ...
      if (!valid) {
        console.log('[janny] invalid signature')
  ...
  helia.libp2p.addEventListener('peer:identify', (event) => {
    const { peerId, protocols } = event.detail
    console.log('[identify] peer', peerId.toString(), 'supports:', protocols)
    console.log('[pubsub] subscribers after identify:', helia.libp2p.services.pubsub.getSubscribers(
      `/chainId/${[...Object.keys(instances)][0]}/address/${instances[[...Object.keys(instances)][0]]?.address}`
    ).map(p => p.toString()))
  })
  ...
  db.events.on('join', (peerId, heads) => {
    console.log('user looking to join orbit b')
    console.log("db:events:on:join", peerId, heads)
  })
